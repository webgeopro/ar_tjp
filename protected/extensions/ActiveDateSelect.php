 ...
 *
 **/
class ActiveDateSelect extends CInputWidget
{
	...
	public function init()
	{
		...
		if(!isset($this->day_format) or empty($this->day_format)) $this->day_format      = "%d";
		/* Write day values using this format MB */
		if(!isset($this->day_value_format) or empty($this->day_value_format)) $this->day_value_format = "%02d";
		...
		if(!isset($this->field_array)) $this->field_array     = $name;
	...
	public function run()
	{
		...
		if (preg_match('!^(\+|\-)\s*(\d+)$!', $this->end_year, $match))
		{
			if ($match[1] == '+')
			{
				$this->end_year = strftime('%Y') + $match[2];
			}
			else
			{
				$this->end_year = strftime('%Y') - $match[2];
		...
		if (preg_match('!^(\+|\-)\s*(\d+)$!', $this->start_year, $match))
		{
			if ($match[1] == '+')
			{
				$this->start_year = strftime('%Y') + $match[2];
			}
			else
			{
				$this->start_year = strftime('%Y') - $match[2];
		...
		{
			if ($this->start_year > $this->time[0] && !isset($this->start_year))
			{
				// force start year to include given date if not explicitly set
				$this->start_year = $this->time[0];
			}
			if($this->end_year < $this->time[0] && !isset($this->end_year))
			{
	...
	public static function conversionHandler($event, $attribute)
	{
		...
		#die(print_r($event->sender->$attribute));
    ...
    public static function only_sanitize($data)
    {
        die(strtr('Year-Month-Day', (array) $data));
    }
    ...
    /**
     * Простая проверка подключения из модулей
     */
	...
	protected function make_timestamp($string)
	{
		...
		elseif (preg_match('/^\d{14}$/', $string))
		{
			...
			substr($string, 4, 2),substr($string, 6, 2),substr($string, 0, 4));//die($time);
		}
