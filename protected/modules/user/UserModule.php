...
class UserModule extends CWebModule
{
    ...
    public $profileEditUrl = array("/user/profile/edit");
	...
	public $tableUsers = 'user_users'; //{{users}}';
	public $tableProfiles = 'user_profiles'; //{{profiles}}';
	public $tableProfileFields = 'user_profiles_fields'; //{{profiles_fields}}';
	...
	public function init()
	{
		...
		$this->setImport(array(
			'user.models.*',
			'user.components.*',
            //'application.extensions.phpass.Phpass', Не используется
		));
	}
     ...
     * @return hash string
     * Изменено для совместимости со старыми записями Gallery2
     */
	public static function encrypting($string="", $userPassword='') {

        if ($userPassword) { // Проверка пароля
            if (36 == strlen($userPassword))
                $salt = substr($userPassword, 0, 4);
            else return false;
            // Проверка пароля
        } else { // Генерация нового пароля
            for ($i = 0, $salt = ''; $i < 4; $i++) {
                $char = mt_rand(48, 109);
                $char += ($char > 90) ? 13 : ($char > 57) ? 7 : 0;
                $salt .= chr($char);
            }
        }//die("\$string=$string \$userPassword=$userPassword");
	...
	public static function sendMail($email,$subject,$message) {
    	$adminEmail = Yii::app()->params['adminEmail']; //text/html
