...
class UserRecoveryForm extends CFormModel {
	public $username, $login_or_email, $user_id;
	...
	{
		return array(
			// username and password are required login_or_email
	...
	public function checkexists($attribute,$params) {
		if(!$this->hasErrors())  // we only want to authenticate when no input errors
		{
			/*if (strpos($this->login_or_email,"@")) {
				...
				$user=User::model()->findByAttributes(array('username'=>$this->username)); //login_or_email
				if ($user)
					$this->user_id=$user->id;
			/*}*/
